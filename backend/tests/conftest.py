...
def clear_session_cross_caches():
    """Prevent _session_cross_caches from leaking state between tests.

    Each test creates a fresh DB whose sessions start at id=1, so without
    this reset, cached tool-call keys from one test contaminate the next.
    """
    import app.agent.orchestrator as orch_mod
    orch_mod._session_cross_caches.clear()
    yield
    orch_mod._session_cross_caches.clear()
