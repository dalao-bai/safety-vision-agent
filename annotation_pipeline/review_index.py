...
REVIEW_ONLY_KEYS = {"decision", "review_note", "revised"}


def review_object_template(obj: dict[str, Any]) -> dict[str, Any]:
    item = dict(obj)
...
def review_template(draft: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "objects": [review_object_template(obj) for obj in draft.get("objects", [])],
        "review_note": "",
    }


def upgrade_review_with_draft(review: dict[str, Any], draft: dict[str, Any]) -> dict[str, Any]:
    draft_by_index = {obj.get("draft_object_index"): obj for obj in draft.get("objects", [])}
    upgraded = dict(review)
    upgraded["sample_id"] = upgraded.get("sample_id") or draft.get("sample_id")
    upgraded["image_path"] = upgraded.get("image_path") or draft.get("image_path")
    upgraded["review_status"] = upgraded.get("review_status") or "pending"
    upgraded["image_decision"] = upgraded.get("image_decision") or "pending"
    upgraded["review_note"] = upgraded.get("review_note", "")

    objects = []
    for item in review.get("objects", []):
        if not isinstance(item, dict):
            continue
        draft_obj = draft_by_index.get(item.get("draft_object_index"), {})
        merged = dict(draft_obj)
        revised = item.get("revised")
        if item.get("decision") == "revise" and isinstance(revised, dict):
            merged.update(revised)
        for key, value in item.items():
            if key not in REVIEW_ONLY_KEYS:
                merged[key] = value
        merged["decision"] = item.get("decision", "pending")
        merged["review_note"] = item.get("review_note", "")
        objects.append(merged)
    upgraded["objects"] = objects
    return upgraded
